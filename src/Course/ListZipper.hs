...
findLeft p (ListZipper l a r) = 
  case break p l of
    (_, Nil) -> IsNotZ
    (x, y :. ys) -> IsZ $ ListZipper ys y (reverse (a :. x) ++ r)
...
findRight p (ListZipper l a r) =
  case break p r of
    (_, Nil) -> IsNotZ
    (xs, y :. ys) -> IsZ $ ListZipper (reverse (a :. xs) ++ l) y ys
...
moveLeftLoop (ListZipper l a r) =
  case l of
    Nil -> let (bs, b) = foldLeft (\(bs', b') a' -> (b' :. bs', a')) (Nil, a) r
            in ListZipper bs b Nil
    (la :. ls) -> ListZipper ls la (a :. r)
...
moveRightLoop (ListZipper l a r) =
  case r of
    Nil -> let (bs, b) = foldLeft (\(bs', b') a' -> (b' :. bs', a')) (Nil, a) l
            in ListZipper Nil b bs
    (ra :. rs) -> ListZipper (a :. l) ra rs
...
nth n z@(ListZipper l _ _)
  | n <= ln = moveLeftN (ln - n) z
  | otherwise = moveRightN (n - ln) z
    where ln = length l
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft b (ListZipper l a r) =
  ListZipper (a :. l) b r
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight b (ListZipper l a r) =
  ListZipper l b (a :. r)
