...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok l r p =
  between (charTok l) (charTok r) p
...
hex ::
  Parser Char
hex = 
  replicateA 4 (satisfy isHexDigit) >>= \s ->
    case readHex s of
...
eof ::
  Parser ()
eof =
  P $ \s -> if isEmpty s then Result s () else ExpectedEof s
