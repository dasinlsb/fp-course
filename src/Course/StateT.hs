...
instance Functor f => Functor (StateT s f) where
  (<$>) ::
    ...
    -> StateT s f a
    -> StateT s f b
  f <$> st = StateT $ \s -> f' (runStateT st s)
    where f' = (<$>) (\(a, s) -> (f a, s))
...
instance Functor f => Functor (OptionalT f) where
  -- g <$> (OptionalT f a) = OptionalT $ pure g a 
...
instance Monad f => Applicative (OptionalT f) where
  pure =
    OptionalT . pure . pure
  f <*> a = OptionalT $ runOptionalT f >>= \f' ->
                        case f' of 
                          Empty -> pure Empty
                          Full _ -> runOptionalT a >>= \a' -> return $ f' <*> a'
...
distinctG xs =
  runOptionalT $ evalT (filtering f xs) S.empty
    where f a = StateT $ \s -> OptionalT $
                if a > 100
                then
                  log1 (fromString "aborting > 100: " ++ show' a) Empty
                else (if P.even a 
                  then log1 (fromString "even number: " ++ show' a)
